...
STEP_AMOUNT = 7     #Side length of polygons
PADDING_FACTOR = 10
...
def create_heatmap(list_of_lat_longs):
        ...
        max_lat = float(max(list_of_lat_longs)[0])
        min_lat = float(min(list_of_lat_longs)[0])
        max_long = float(max(list_of_lat_longs)[1])
        min_long = float(min(list_of_lat_longs)[1])
...
##        min_long = dest_min.longitude
    ...
    #calculate a_values for each center
        ...
        for center in center_points:
                print("center no." + str(center_points.index(center))+"/"+str(len(center_points)))
                a_val = 0
                for point in list_of_lat_longs:
                        #print("\tpoint no."+str(list_of_lat_longs.index(point))+"/"+str(len(list_of_lat_longs)))
